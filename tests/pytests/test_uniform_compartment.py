...
def test_single_compartment_react_variable_steps_1():
    ...
    # Elementary unimolecular reaction S <-> X , with mass-action kinetics (mostly forward)
...
def test_single_compartment_correct_neg_conc():
    ...
    # Unimolecular elementary reaction S <-> X , with mass action kinetics (mostly forward)
    uc.add_reaction(reactants="S", products="X",
                    reaction_model="mass action", kinetic_parameters={"kF": 6., "kR": 3.})
...
def test_reaction_in_equilibrium():
    ...
    # Reaction 0:  2A <-> B , NOW WITH 2nd-order kinetics in the forward direction (i.e. regular mass-action kinetics)
    uc.add_reaction(reactants=[(2, "A")], products=["B"],
                    reaction_model="mass action", kinetic_parameters={"kF": 5., "kR": 2.})
