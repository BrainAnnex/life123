...
from life123.bio_sim_1d_experimental import BioSim1D_NEW
...
def test_constructor():
    with pytest.raises(Exception):
        BioSim1D_NEW()              # Missing required arguments

    with pytest.raises(Exception):
        BioSim1D_NEW(n_bins="I'm not an integer")

    with pytest.raises(Exception):
        BioSim1D_NEW(n_bins=0)      # Must be at least 1

    with pytest.raises(Exception):
        BioSim1D_NEW(n_bins=3)      # At least one more arg is needed
    ...
    with pytest.raises(Exception):
        BioSim1D_NEW(n_bins=5, reaction_handler=rxn)    # No chemicals yet registered


    # New test
    rxn = UniformCompartment(names=["A", "B", "C"])
    bio = BioSim1D_NEW(n_bins=5, reaction_handler=rxn)
    bio.describe_state()
    assert bio.n_bins == 5
    assert bio.n_species == 3
