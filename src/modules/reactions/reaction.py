...
class Reaction:     #TODO: in progress
    ...
    def __init__(self, names=None, diffusion_rates=None, n_species=None):
        ...
        self.reaction_list = []     # List of dicts.  Each item represents a reaction, incl. its reverse
                                    ...
                                    #       "reactants", "products",
                                    #       "kF", "kR", "Delta_H", "Delta_S", "Delta_G", "K", "enzymes"
                                    #       (See the class heder for more details)
                                    # TODO: maybe turn each reaction into an object
    ...
    def extract_rxn_properties(self, rxn: dict) -> {}:
        ...
        for k,v in rxn.items():
            if k not in ("reactants", "products", "enzymes") and (v is not None):
                properties[k] = v
    ...
    def add_reaction(self, reactants: Union[int, str, tuple, list], products: Union[int, str, tuple, list],
        ...
        :return:                None (the object variable self.reaction_list gets appended to)
        """
        ...
        assert reactant_list != product_list, \
            f"ReactionData.add_reaction(): the reactants and the products can't all be identical! " \
        ...
        if number_enzymes == len(reactant_list) or number_enzymes == len(product_list):
            raise Exception("ReactionData.add_reaction(): all terms in the reaction appear to be enzymes!")


        rxn = {"reactants": reactant_list, "products": product_list,
               "kF": None, "kR": None,
               "Delta_H": None, "Delta_S": None, "Delta_G": None,
               "K": None, "enzymes": enzyme_list
               }
